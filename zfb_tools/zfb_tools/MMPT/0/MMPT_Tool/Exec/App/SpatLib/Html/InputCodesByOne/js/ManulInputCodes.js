...
function Format(obj){
	...
	else if(-1 != strName.search(/ENETMAC/g) || -1 != strName.search(/BT/g)){
		$(obj).val($(obj).val().replace(/[^0-9|^a-f|^A-F|^-]/g,''));
	}	
}
...
function ValidateBarcode(sort){
	...
	if(-1 != Settings.Barcodes[sort].Name.search(/SN/g)){
    ...
    $(".BarcodeInputBox").keyup(function (event) {
		...
		if ($("#" + Settings.Barcodes[sort].Name).val().length == 14){
			...
			if(!validateInputLength(Settings.Barcodes[sort].Name, Settings.Barcodes[sort].MaxLength)){
				$("#BarcodeError").html(Settings.Barcodes[sort].Name + " length is wrong!");
			...
			document.getElementById($(".BarcodeName")[sort].id).style.display="none";	
			document.getElementById($(".BarcodeInputBox")[sort].id).style.display="none";		
			...
			do{
				...
				if(Settings.Barcodes[sort].Enable && Settings.Barcodes[sort].GenerateType == "ManualInput"){
					document.getElementById($(".BarcodeName")[sort].id).style.display="inline";	
					document.getElementById($(".BarcodeInputBox")[sort].id).style.display="inline";						
					$("#" + Settings.Barcodes[sort].Name).focus();
					$("#" + Settings.Barcodes[sort].Name).select();
					return;
				}
