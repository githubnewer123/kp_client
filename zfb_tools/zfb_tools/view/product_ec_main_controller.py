...
AUTH_CFG = {"username": "EC618@zyxa", "password": "11447733", "pname": "XZX", "cname": "CIS_MODULE"}
class ProductEcMainController(QObject):
    TYPE_CAL  = 0x01
    TYPE_NST = 0x02
    TYPE_CAL_NST = 0x03
    ...
    def copyCalLog(self,slot_index, folder, imei, upload_path):
        ...
        # save upload log  EMAT/log/%d/UPLOAD_IMEI
        # upload_path = os.path.join(log_path, "UPLOAD_" + imei)
        # if not os.path.exists(upload_path):
        #     os.makedirs(upload_path)
        # if int(self.config.station_type) == 0:
        for file in os.listdir(folder):
            if "Rf_Cal_Nst_Log.txt" in file and file.endswith("txt"):
    ...
    def uploadLogFolder(self,src, imei, chip_id,res):
        # src = "%sEMAT/log/0/UPLOAD_%s"%(Utils.getAppRootPath(), imei)
        if int(self.config.station_type) == 0:
            tag = "cal"
        else:
            tag = "nst"
        dst = "%sEMAT/log/0/%s_%s_%s_%s.zip" % (
            Utils.getAppRootPath(), tag, imei, datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'),
    ...
    def __productThread(self):
        ...
        if cal_lte or nst_lte :
            try:
                self.mslot = EcSlot(EcSlot.PROCESS_LTE_CAL if cal_lte else EcSlot.PROCESS_LTE_NST, self.config)
            except Exception as e:
                self.system_fault_signal.emit({"info":str(e)})
        ...
        while True:
            ...
            process_res = False
            mmpt_res = EcSlot.PROCESS_SUCCESS
            false_info = ""
            product_res.scrip_version = self.mes.getActiveOrder().scrip_version
            try:
                emat_log_path = os.path.join(
                    os.path.join(Utils.getAppRootPath(), "EMAT/log/%d/UPLOAD_%s" % (0, imei)))
                if os.path.exists(emat_log_path):
                    shutil.rmtree(emat_log_path)
                # cal_log_path = os.path.join(
                #     os.path.join(Utils.getAppRootPath(), "EMAT/%d/Data/Log" % (0)))
                cal_log_path = r"C:\EigenComm_LogStorage"

                type = int(self.config.station_type)
                if (type == 0 and self.config.cal_imei) or (type == 1 and self.config.cal_imei):
                    if self.config.check_imei_uniq:
                        self.mes.getChipidFromMes(imei)
                if type == 2:
                    self.signalEmit("等待烧录完成")
                    process_res = False
                    res,chip_id, false_info = self.fw_downloader.waitFinish(50)
                    product_res.chip_id = chip_id
                    if res is False:
                        raise Exception(false_info)
                else:
                    if cal_lte or nst_lte:
                        self.signalEmit("等待EMAT TOOL返回结果 IMEI: %s" % (imei))
                        if os.path.exists(cal_log_path):
                            shutil.rmtree(cal_log_path)

                        self.mslot.stop()
                        self.mslot.start(at_port)
                        emat_res, msg = self.mslot.waitComplete()
                        if emat_res == False:
                            dut = EcDut(at_port, update_authid=False)
                            product_res.chip_id = dut.getChipID()
                            dut.destory()
                            raise Exception("EMAT: cal error %s" % (msg))

                    dut = EcDut(at_port, update_authid = self.config.cal_auth)
                    product_res.chip_id = dut.getChipID()
                    ...
                    if type == 0:
                        if self.config.cal_imei:
                            if self.config.check_imei_uniq:
                                ...
                                if self.mes.chipid_list:
                                    if len(
                                            self.mes.chipid_list) == 1 and product_res.chip_id not in self.mes.chipid_list:
                        ...
                        if self.config.cal_sn:
                            ...
                            batch_no = dut.setHwVersion(self.mes.getActiveOrder().project_version)
                    ...
                    if type == 1:
                        ...
                        if self.config.test_io:
                            ...
                            res, data = dut.checkGPIOExt(self.mes.getActiveOrder().gpio_mask, "800EG" if "800-EG" in self.mes.getActiveOrder().project_version else "600EG")
                        ...
                        if "D30G" in self.mes.getActiveOrder().fw_version_name:
                            dut.checkRndisEnumEnable()
                            print("RNDIS网卡检查成功")
            ...
            try:
                for path1 in os.listdir(cal_log_path):
                    # path1 success/fail
                    if os.path.isdir(os.path.join(cal_log_path, path1)):
                        for path2 in os.listdir(os.path.join(cal_log_path, path1)):
                            # path2 thread1
                            log_path1 = os.path.join(os.path.join(cal_log_path, path1), path2)
                            for path3 in os.listdir(log_path1):
                                log_path = os.path.join(log_path1, path3)
                                if os.path.exists(log_path):
                                    self.copyCalLog(0, log_path, imei, emat_log_path)
                                    break
                if os.path.exists(emat_log_path):
                    product_res.log_url = self.uploadLogFolder(emat_log_path ,imei, product_res.chip_id, emat_res)
    ...
    def getLogPath(self, slot_index):
        log_path = os.path.join(Utils.getAppRootPath(), "EMAT/log/%d/" % (slot_index))
        if not os.path.exists(log_path):
