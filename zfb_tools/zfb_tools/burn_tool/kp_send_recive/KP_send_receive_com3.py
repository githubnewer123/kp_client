...
class Kp_send_com3(QObject):
    signal_send_freq_com3 = pyqtSignal(str) #定义一些发送的信号槽
    signal_send_pass_freq_com3 = pyqtSignal(str)
    signal_send_cos_fail_freq_com3 = pyqtSignal(str)
    signal_send_kr_fail_freq_com3 = pyqtSignal(str)
    signal_send_connect_com3 = pyqtSignal(str)
    signal_send_SuperText_com3 = pyqtSignal(str)
    signal_send_Progress_com3 = pyqtSignal(str)
    signal_send_pass_fail_com3 = pyqtSignal(str)
    signal_send_connect_messagebox_com3 = pyqtSignal(str)
    ...
    def set_logger(self):
        # 配置日志基本设置
        self.logger_com2 = logging.getLogger(__name__)
        self.logger_com2.setLevel(logging.DEBUG)
        ...
        self.file_handler_com2 = logging.FileHandler(
            'log/kr_{}_com3.log'.format(datetime.datetime.now().strftime('%Y-%m-%d_%H_%M_%S')))
        self.file_handler_com2.setLevel(logging.DEBUG)
        self.file_handler_com2.setFormatter(formatter)
        self.logger_com2.addHandler(self.file_handler_com2)
        # 创建一个流处理器，将错误级别的日志消息输出到命令行
        stream_handler = logging.StreamHandler()
        stream_handler.setLevel(logging.ERROR)
        stream_handler.setFormatter(formatter)
        self.logger_com2.addHandler(stream_handler)
        ...
        try:
            ...
            if res_con:
                if res_con[0:12] == '5253a500000e':
                    ...
                    self.signal_send_connect_com3.emit('{}'.format(self.port))
                    self.signal_send_SuperText_com3.emit('{} 串口已连接，通信正常'.format(self.get_cur_time()))
                    self.Data_dispose()  # 进入Kr烧入循环
                else:
                    self.my_function('串口未连接，通信异常')
                    self.signal_send_SuperText_com3.emit('{} 串口未连接，通信异常'.format(self.get_cur_time()))
                    self.signal_send_connect_messagebox_com3.emit("连接出错")  # mssagebox框提示
                    self.signal_send_connect_com3.emit("已关闭")
                    self.dev.close()
            ...
            print("串口错误:", e)
            self.signal_send_connect_com3.emit("已关闭")
            self.signal_send_connect_messagebox_com3.emit(f"串口错误:{e}")
            self.dev.close()
        except Exception as e:
            self.logger_com2.exception('错误：{}'.format(e))
            sys.exit()###退出线程
        ...
        while not self._stop_event.is_set():  # 检查停止事件是否被设置
            ...
            if self.data_huk:
                if self.data_huk[:12] == '5253b0000010':
                    ...
                    self.burn_result = 'COM位3 COS烧入失败 次数{}'.format(self.cos_fail_freq)
                    self.signal_send_cos_fail_freq_com3.emit(str(self.cos_fail_freq))  # 发送失败拉取次数
                    self.signal_send_pass_fail_com3.emit('FAIL')  # pass fail判断
                    print(self.get_cur_time(), self.burn_result) #终端打印失败拉取次数
                    self.my_function(self.burn_result) #log打印失败拉取次数
                    self.signal_send_SuperText_com3.emit('{} COS烧入失败 次数{}'.format(self.get_cur_time(),self.cos_fail_freq)) #超级文本打印失败拉取次数
                    self.signal_send_freq_com3.emit(str(self.kr_fail_freq + self.cos_fail_freq + self.pass_freq))  # 文本显示总次数
                    self.my_function('烧入总次数{}'.format(str(self.kr_fail_freq + self.cos_fail_freq + self.pass_freq))) #log打印总拉取次数
                    print('-----'*40)
                    self.my_function('-----'*40)
                    self.signal_send_SuperText_com3.emit('-----'*40)
                    ...
                    self.fail_freq = self.kr_fail_freq + self.cos_fail_freq
                    self.burn_result = 'COM位3 kr烧入失败 次数{}'.format(self.kr_fail_freq)
                    self.signal_send_kr_fail_freq_com3.emit(str(self.kr_fail_freq))  # 发送失败拉取次数
                    self.signal_send_pass_fail_com3.emit('FAIL')  # pass fail判断
                    print(self.get_cur_time(), self.burn_result) #终端打印失败拉取次数
                    self.my_function(self.burn_result) #log打印失败拉取次数
                    self.signal_send_SuperText_com3.emit('{} kr烧入失败 次数{}'.format(self.get_cur_time(),self.kr_fail_freq)) #超级文本打印失败拉取次数
                    self.signal_send_freq_com3.emit(str(self.cos_fail_freq + self.kr_fail_freq + self.pass_freq))  # 文本显示总次数
                    ...
                    self.fail_freq = self.kr_fail_freq + self.cos_fail_freq
                    self.burn_result = 'COM位3 kr烧入成功 次数{}'.format(self.pass_freq)
                    self.signal_send_pass_freq_com3.emit(str(self.pass_freq))  # 发送成功拉取次数
                    self.signal_send_pass_fail_com3.emit('PASS')  # pass fail判断
                    print(self.get_cur_time(), self.burn_result) #终端打印失败拉取次数
                    self.my_function(self.burn_result) #log打印失败拉取次数
                    self.signal_send_SuperText_com3.emit('{} kr烧入成功 次数{}'.format(self.get_cur_time(),self.pass_freq)) #超级文本打印失败拉取次数
                    self.my_function('烧入总次数{}'.format(str(self.kr_fail_freq + self.cos_fail_freq + self.pass_freq))) #log打印总拉取次数
                    # self.signal_send_Progress_com3.emit(str(self.pass_freq))  # 进度条显示成功次数的进度
                    self.signal_send_freq_com3.emit(str(self.kr_fail_freq + self.cos_fail_freq + self.pass_freq))  # 文本显示总次数
                    print('-----'*40)
                    self.my_function('-----'*40)
                    self.signal_send_SuperText_com3.emit('-----'*40)
                    ...
                    self.signal_send_SuperText_com3.emit(
                    ...
                    self.signal_send_SuperText_com3.emit('{} No data was huk，Data anomalies！！！'.format(self.get_cur_time()))
                    self.signal_send_connect_com3.emit("已关闭")
                    self.dev.close()
                ...
                self.signal_send_SuperText_com3.emit('{} 停止读取数据'.format(self.get_cur_time()))
                self.signal_send_connect_com3.emit("已关闭")
                self.dev.close()
                raise Exception("停止读取数据") #抛出没有数据的错误

            # self.total_times = self.fail_freq + self.pass_freq
            # self.signal_send_freq_com3.emit(str(self.total_times))  # 文本显示总次数
            # self.my_function('烧入总次数{}'.format(str(self.total_times))) #log打印总拉取次数
            # self.signal_send_SuperText_com3.emit('{} kr烧入 总次数{}'.format(self.get_cur_time(), self.total_times))  # 超级文本打印总次数
    ...
    def kr_burn_in(self):
        ...
        if isinstance(kr, dict):
            if kr['ret'] == 0:
                ...
                self.signal_send_SuperText_com3.emit('{} kr拉取完成'.format(self.get_cur_time()))
    ...
    def writeCmd(self, dev, cmd):
            ...
            self.signal_send_SuperText_com3.emit('{} 发送：{} \r\n{} 等待读取数据...'.format(self.get_cur_time(), cmd.hex(),self.get_cur_time()))
            ...
            self.signal_send_SuperText_com3.emit('{} 收到:{}'.format(self.get_cur_time(),hex_data))
            return hex_data

    def readhuk(self):
        print(self.get_cur_time(), '等待读取huk或判断指令...')
        self.signal_send_SuperText_com3.emit('{} 等待读取huk或判断指令...'.format(self.get_cur_time()))
        ...
        self.signal_send_SuperText_com3.emit('{} 收到huk或判断指令:{}'.format(self.get_cur_time(), huk_hex))
        self.my_function('收到huk或判断指令:{}'.format(huk_hex))
        return huk_hex

    def my_function(self,info):
        self.logger_com2.debug(info)
    ...
    def test_pull(self, pull_model, pull_license, pull_count, chip_type, huks_in):
        ...
        if self.HOST != b"http://10.1.100.66:28080":
            self.signal_send_SuperText_com3.emit('网络超时，请检查网络地址')
    ...
    def dumpHSecResult(self, pResult):
        ...
        self.signal_send_SuperText_com3.emit('服务器拉取:{}'.format(out))
        return out
    ...
    def getAtPortList1(self, filter="USB"):
        ...
        print(self.get_cur_time(), 'com口列表:', list_com)
        self.my_function('com口列表：{}'.format(list_com))
        return list_com
